...
const userSchema = new mongoose.Schema(
  {
    ...
    bio: {
      type: String,
      default:
        "AI-powered cultural analyst with a passion for discovering emerging trends",
    },
    tier: {
      type: String,
    ...
    aiInsights: {
      lastPersonalityAnalysis: Date,
      lastTrendPrediction: Date,
...
userSchema.methods.updateCompatibilityScore = function () {
  ...
  this.tasteProfile.compatibility_score = Math.min(
    10,
    avgRating * 2 + interactionBonus
  );
};
  ...
  history.forEach((item) => {
    ...
    if (item.rating) {
      ...
      categoryStats[item.category].avgRating =
        categoryStats[item.category].totalRating /
        categoryStats[item.category].count;
    }
  });
